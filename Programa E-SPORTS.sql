-- PROGRAMA E-SPORTS LEAGUE OF LEGENDS
        ...
        EXIT WHEN v_contador = v_maximo - 1;
            ...
            v_equipo2.cod_equipo := v_equipo1.cod_equipo;
                ...
                WHILE v_equipo2.cod_equipo < v_maximo LOOP
                    -- EL CÓDIGO DE PARTIDO VA INCREMENTANDO EN 1
                    v_partido := v_partido + 1;
                    -- EL CÓDIGO DEL EQUIPO2 SERÁ SIEMPRE 1 MAYOR QUE EL ANTERIOR, HASTA LLEGAR AL ÚLTIMO EQUIPO
...
BEGIN
    ...
    END LOOP;
    -- BUCLE QUE VA RECORRIENDO TODOS LOS PÁRTIDOS Y SUMANDO 1 VICTORIA AL GANADOR EN LA TABLA 'RANKING'
    FOR v_resultado IN c_resultados LOOP
        UPDATE ranking
        SET victorias = victorias + 1
        -- CUANDO EL RESULTADO COINCIDE CON EL CÓDIGO DEL EQUIPO DE LA TABLA 'RANKING', SE LE SUMA 1 VICTORIA
...
DECLARE
    ...
    -- VARIABLES QUE GUARDAN LOS CÓDIGOS DE LOS 2 PRIMEROS EQUIPOS DEL RANKING SEGÚN SU PUESTO
    v_primero ranking.cod_equipo%TYPE;
    v_segundo ranking.cod_equipo%TYPE;
    -- VARIABLES QUE GUARDAN LAS VICTORIAS DE LOS 2 PRIMEROS EQUIPOS PARA EL DESEMPATE
...
BEGIN
    -- PEDIMOS POR TECLADO EN NOMBRE DE LA COMPETICIÓN  
    ...
    FOR v_partidos IN c_partidos LOOP
        ...
        -- GENERAMOS UN NÚMERO ALEATORIO ENTRE 1 y 2, SI SALE 1 GANA EL EQUIPO1 Y SI SALE 1 GANA EL EQUIPO2
    ...
    -- 4.OBTENEMOS LOS CÓDIGOS DE LOS 2 PRIMEROS EQUIPOS DEL RANKING Y LO GUARDAMOS EN VARIABLES
    SELECT obtener_posicion(1) INTO v_primero FROM DUAL;
    SELECT obtener_posicion(2) INTO v_segundo FROM DUAL;
    -- GUARDAMOS EN LAS VARIABLES LA VICTORIAS DE LOS 2 PRIMEROS EQUIPOS
